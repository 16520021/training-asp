...
function generateData(input, type) {
  for (let i = 0; i < input.length; i += 1) {
    ...
    if (type === _utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].FOLDER_TYPE) attachOnclickFolder(id, row);
  ...
  btn.onclick = function () {
    ...
    if (!editMode) {
      if (isFile) {
        ...
        if (clickedRow == '0') folder.parentID = null;else folder.parentID = parseInt(clickedRow);
      ...
      if (isFile) {
        Object(_utilities_API_fetch__WEBPACK_IMPORTED_MODULE_6__["getData"])(_utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_API_URL + 'File/GetFileById/' + hoverRow, {}).then(data => {
          ...
          Object(_utilities_API_fetch__WEBPACK_IMPORTED_MODULE_6__["putData"])(_utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_API_URL + 'File/CreateOrEditFile?', data).finally(() => refresh());
        });
      } else {
        Object(_utilities_API_fetch__WEBPACK_IMPORTED_MODULE_6__["getData"])(_utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_API_URL + 'Folder/GetFolderById/' + hoverRow, {}).then(data => {
          ...
          data.modifiedAt = Object(_utilities_utilities_function__WEBPACK_IMPORTED_MODULE_4__["getCurrentDate"])();
          data.modifiedBy = "Thinh tre trau";
          Object(_utilities_API_fetch__WEBPACK_IMPORTED_MODULE_6__["putData"])(_utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_API_URL + 'Folder/CreateOrEditFolder?', data).finally(() => refresh());
        });
      }
...
function backEvent() {
  document.getElementById('back-btn').onclick = function () {
    if (clickedRow !== _utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_DIRECTORY) {
      Object(_utilities_API_fetch__WEBPACK_IMPORTED_MODULE_6__["getData"])(_utilities_constant__WEBPACK_IMPORTED_MODULE_5__["properties"].BASE_API_URL + 'Folder/GetFolderById/' + clickedRow, {}).then(data => {
        if (data.parentID != null) clickedRow = data.parentID;else clickedRow = '0';
        refresh();
      });
    }
  };
...
const properties = {
  CREATE_MODE: 'create',
  EDIT_MODE: 'edit',
  ORDERING: ['icon', 'folderName', 'fileName', 'createAt', 'modifiedBy', 'id'],
  BASE_DIRECTORY: null,
  FILE_TYPE: 'file',
  FOLDER_TYPE: 'folder',
  FILE_DEFAULT_URL: './dist/image/excel.png',
  FOLDER_DEFAULT_URL: './dist/image/folder.png',
  BASE_API_URL: 'https://localhost:44392/api/'
};
